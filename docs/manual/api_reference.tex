...
\texttt{select\_distribution(name)} &
    Manually overrides the selected distribution. \\
...
\texttt{sample\_mixed(zero\_prop, one\_prop, size)} &
...
\texttt{analyze\_results(quantiles, **kwargs)} &
...
\midrule
\endhead
\texttt{policies\_df}   & Policy table with required columns. \\
...
\texttt{simulate\_dates\_nhpp(lambda0, alpha, phase, T)} &
...
% =====================================================================
\section{ActuarialDistribution Names}
\label{api:dist_names}

The following table lists the distribution names accepted by both
\texttt{DistributionFitter} and \texttt{StochasticSimulator}.
